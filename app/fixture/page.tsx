...
export default function FixturePage() {
  ...
  const handleSave = (matchId: string, homeGoals: number, awayGoals: number) => {
    ...
    const updated: Tournament = {
      ...tournament,
      matches,
      status: allPlayed ? 'finished' : 'active',
    };
  ...
  if (tournament.participants.length === 0) {
    return (
      <EmptyState
        ...
        desc="Primero agregá los jugadores del torneo."
        linkHref="/participantes"
        linkLabel="Ir a Participantes"
      />
    );
  }
  ...
  if (tournament.matches.length === 0) {
    return (
      <EmptyState
        ...
        desc="Generá el fixture desde la sección de Participantes."
        linkHref="/participantes"
        linkLabel="Ir a Participantes"
      />
    );
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="w-48">
          <div className="flex justify-between text-xs text-gray-400 mb-1">
            <span>Progreso</span>
            <span>{total > 0 ? Math.round((played / total) * 100) : 0}%</span>
          </div>
      ...
      {tournament.status === 'finished' && (
        <div className="bg-green-900/40 border border-green-600 rounded-xl p-4 text-center">
          ...
          <p className="text-green-300 font-bold mt-1">¡Torneo finalizado! Todos los partidos están jugados.</p>
          <Link href="/posiciones" className="inline-block mt-2 text-sm text-green-400 hover:text-green-300 underline">
          ...
          <div key={round} className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-gray-500">
                {roundPlayed}/{matches.length} jugados
              </span>
...
function EmptyState({ icon, title, desc, linkHref, linkLabel }: {
  ...
  return (
    <div className="text-center py-20 space-y-4">
      ...
      <Link href={linkHref} className="inline-block bg-green-600 hover:bg-green-500 text-white font-bold px-5 py-2 rounded transition-colors">
        {linkLabel}
      </Link>
    </div>
  );

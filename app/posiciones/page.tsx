    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">{total === 0 ? 'Sin fixture generado aún.' : `${played} de ${total} partidos jugados`}</p>
        </div>
        {tournament.status === 'finished' && (
          <span className="bg-yellow-500/20 border border-yellow-500 text-yellow-300 text-xs font-bold px-3 py-1 rounded-full">🏆 Torneo finalizado</span>
        )}
      </div>
      ...
      {tournament.participants.length === 0 ? (
        <div className="text-center py-16 space-y-3">
          ...
          <Link href="/participantes" className="inline-block bg-green-600 hover:bg-green-500 text-white font-bold px-5 py-2 rounded text-sm transition-colors">Ir a Participantes</Link>
        </div>
      ) : (
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-600">
        <span>PJ = Jugados</span><span>PG = Ganados</span><span>PE = Empatados</span>
        <span>PP = Perdidos</span><span>GF = Goles a favor</span><span>GC = Goles en contra</span>
        <span>DG = Diferencia de gol</span><span>PTS = Puntos</span>
      </div>
      ...
      {withGames.length > 0 && (
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {standings[0] && <HighlightCard icon="🥇" label="Líder" value={standings[0].participant.name} sub={`${standings[0].points} puntos`} />}
            {topScorer && <HighlightCard icon="⚽" label="Máximo goleador" value={topScorer.participant.name} sub={`${topScorer.goalsFor} goles`} />}
            {bestDef && <HighlightCard icon="🛡️" label="Mejor defensa" value={bestDef.participant.name} sub={`${bestDef.goalsAgainst} en contra`} />}
          </div>
        </div>
      )}
    </div>
